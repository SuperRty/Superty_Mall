...
@celery_app.task(bind=True, name='send_email_verify', retry_backoff=3)
def send_email_verify(self, to_email, verify_url):
    """
    发送验证邮箱邮件
    ...
    content = '<p>尊敬的用户您好！</p>' \
                   '<p>感谢您使用接收该邮件。</p>' \
                   '<p>您的邮箱为：%s 。请点击下方验证邮箱：</p>' \
                   '<p><a href="%s">点击此处验证邮箱：%s<a></p>' % (to_email, verify_url, verify_url)
